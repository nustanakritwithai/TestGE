import {createWorld,allEntities,stateHash,executeTick,snapshot,makeDamageProposal,resolveConflicts,verify,commit,computeExact,computeApprox} from './twa-core.js';
...
function injectDamage(){const s=snapshot(world),a=makeDamageProposal(s,{target:1,amount:17,source:0,cause:'TWA demo'}),b=makeDamageProposal(s,{target:1,amount:9,source:2,cause:'simultaneous hit'});const physics=computeExact(s),resolved=resolveConflicts([physics,a,b]),check=verify(world,resolved,{critical:true});if(!check.ok){$('log').textContent='Damage transaction blocked: '+check.errors.join(', ');return;}const rec=commit(world,resolved,{verifierResult:check});$('log').textContent=`Transaction demo: 2 damage proposals ถูก combine แบบ deterministic • conflicts=${rec.conflicts}`;draw();}
...
function benchmark(){const reps=+$('reps').value,seeds=+$('seeds').value,rows=[];for(const count of [1000,10000,50000])for(const mode of ['exact','approx']){const times=[];for(let s=0;s<seeds;s++){for(let r=0;r<reps;r++){const w=createWorld({seed:9000+s,count,density:.45,speed:1});const t0=performance.now();executeTick(w,{mode,critical:false});times.push(performance.now()-t0);}}times.sort((a,b)=>a-b);const p=x=>times[Math.floor((times.length-1)*x)];rows.push({count,mode,p50:p(.5),p95:p(.95),p99:p(.99)});} $('benchRows').innerHTML=rows.map(r=>`<tr><td>${r.count.toLocaleString()}</td><td>${r.mode}</td><td>${r.p50.toFixed(3)}</td><td>${r.p95.toFixed(3)}</td><td>${r.p99.toFixed(3)}</td></tr>`).join('');}
