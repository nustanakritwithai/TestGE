...
const TERRAIN=[
 ...
 {x:-.12,y:.38,r:.18,cover:.22,name:'FOREST'},
 {x:.12,y:-.12,r:.13,cover:.28,name:'TRENCH'},
 {x:.14,y:.25,r:.12,cover:.30,name:'TRENCH'}
];
...
function pushLog(type,text){log.push({tick:world?.tick||0,type,text});if(log.length>100)log.shift();$('log').innerHTML=log.slice(-16).reverse().map(x=>`<div class="entry"><b>${type==='CONTACT'?'⚔️ ':''}${x.type}</b> • ${x.text}<div class="muted">tick ${x.tick}</div></div>`).join('')}
function coverAt(x,y,team){let c=0;for(const t of TERRAIN)if(Math.hypot(x-t.x,y-t.y)<t.r)c=Math.max(c,t.cover);if(team===1&&x>.02&&Math.abs(y)<.55)c=Math.max(c,.10);return c}
...
function init(n=count){
 ...
 pushLog('SYSTEM',`Total Front P4 • Company Tactical AI • ${count.toLocaleString()} entities`);
 pushLog('AI','Companies coordinate FIREBASE / MANEUVER / RESERVE / REINFORCE automatically');
 draw();updateHUD();
}
...
function buildHierarchy(team,start,end){
 ...
 for(let b=0;b<bnCount;b++){
  ...
  hierarchy.battalions.push({id,team,start:bs,end:be,homeX:side*.72,homeY:lane,targetX:team===0?-.5:.13,targetY:lane,order:team===0?'ASSEMBLE':'DEFEND',damageDone:0,losses:0});
...
function teamNearHill(team){let n=0;for(let i=0;i<count;i++)if(alive(i)&&world.team[i]===team&&Math.hypot(world.x[i],world.y[i])<.18)n++;return n}
...
function autonomousCommand(){
 ...
 for(const bn of hierarchy.battalions){
  ...
  if(bn.team===0){
   ...
   else if(tick<18){bn.order='ASSEMBLE';bn.targetX=-.50;bn.targetY=bn.homeY}
   else{bn.order=doctrine==='AGGRESSIVE'?'ASSAULT':doctrine==='CAUTIOUS'?'BOUND_ADVANCE':'ATTACK';bn.targetX=.025;bn.targetY=clamp(bn.homeY*.27+laneBias,-.58,.58)}
  }else{
   const pressure=teamNearHill(0)>12;
   if(st.strength<.27||st.morale<17){bn.order='FALLBACK';bn.targetX=.62;bn.targetY=bn.homeY}
   else if(pressure&&st.morale>50&&local%3===0){bn.order='COUNTER';bn.targetX=0;bn.targetY=bn.homeY*.18}
   else{bn.order='DEFEND';bn.targetX=.10+(local%2)*.025;bn.targetY=clamp(bn.homeY*.40,-.56,.56)}
  }
 }
...
function updateCompanyAI(){
 for(const bn of hierarchy.battalions){
  const companies=hierarchy.companies.filter(c=>c.battalionId===bn.id),stats=companies.map(companyStats);
  if(!companies.length)continue;
  if(bn.order==='RETREAT'||bn.order==='FALLBACK'){
   companies.forEach(c=>{c.state=CO.FALLBACK;c.targetX=bn.homeX;c.targetY=bn.homeY+c.offsetY});continue;
  }
  if(bn.team===0){
   let reserveIndex=companies.length-1;
   let pressured=-1,maxSupp=-1;stats.forEach((s,i)=>{if(s.supp>maxSupp){maxSupp=s.supp;pressured=i}});
   companies.forEach((c,i)=>{
    if(i===reserveIndex&&stats[i].strength>.55){c.state=CO.RESERVE;c.targetX=bn.targetX-.10;c.targetY=bn.targetY+c.offsetY*.7}
    else if(i===0){c.state=CO.FIREBASE;c.targetX=bn.targetX-.035;c.targetY=bn.targetY+c.offsetY*.45}
    else{c.state=CO.MANEUVER;const flank=((bn.id+i)%2?1:-1);c.targetX=bn.targetX+.015;c.targetY=clamp(bn.targetY+flank*.11+c.offsetY*.25,-.62,.62)}
   });
   if(pressured>=0&&maxSupp>55&&reserveIndex!==pressured){companies[reserveIndex].state=CO.REINFORCE;companies[reserveIndex].targetX=stats[pressured].x-.035;companies[reserveIndex].targetY=stats[pressured].y}
  }else{
   let weak=0,minStrength=2;stats.forEach((s,i)=>{if(s.strength<minStrength){minStrength=s.strength;weak=i}});
   companies.forEach((c,i)=>{c.state=CO.DEFEND;c.targetX=bn.targetX+(i%2)*.025;c.targetY=clamp(bn.targetY+c.offsetY*.48,-.60,.60)});
   const reserve=companies.length-1;
   if(minStrength<.62&&reserve!==weak){companies[reserve].state=CO.REINFORCE;companies[reserve].targetX=stats[weak].x+.025;companies[reserve].targetY=stats[weak].y}
  }
 }
}
...
function draw(){if(!world)return;const w=canvas.width,h=canvas.height,sx=x=>(x+1)*.5*w,sy=y=>(y+1)*.5*h;ctx.clearRect(0,0,w,h);ctx.fillStyle='#0b1925';ctx.fillRect(0,0,w,h);ctx.strokeStyle='#1e3447';for(let x=0;x<=10;x++){ctx.beginPath();ctx.moveTo(x*w/10,0);ctx.lineTo(x*w/10,h);ctx.stroke()}for(let y=0;y<=8;y++){ctx.beginPath();ctx.moveTo(0,y*h/8);ctx.lineTo(w,y*h/8);ctx.stroke()}
 for(const t of TERRAIN){ctx.fillStyle=t.name==='FOREST'?'#294b36':'#5b4932';ctx.globalAlpha=.65;ctx.beginPath();ctx.arc(sx(t.x),sy(t.y),t.r*.5*w,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;ctx.fillStyle='#c9d8c5';ctx.font='11px system-ui';ctx.fillText(t.name,sx(t.x),sy(t.y))}
 ctx.fillStyle='#6c5934';ctx.beginPath();ctx.arc(sx(0),sy(0),56,0,Math.PI*2);ctx.fill();ctx.fillStyle='#e4c677';ctx.font='bold 18px system-ui';ctx.textAlign='center';ctx.fillText('▲ HILL 42',sx(0),sy(0)+6);
