...
let activity={shots:0,heals:0,grenades:0,scoutShots:0,mgBursts:0,engaged:0,kills:0};
...
function roleRange(r){return r===ROLE.SCOUT?.19:r===ROLE.MARKSMAN?.22:r===ROLE.MG?.17:r===ROLE.GRENADIER?.15:r===ROLE.LEADER?.145:.14}
function roleDamage(r){return r===ROLE.MARKSMAN?10:r===ROLE.MG?4.2:r===ROLE.GRENADIER?7:r===ROLE.SCOUT?4:r===ROLE.LEADER?5:5.2}
function roleSuppression(r){return r===ROLE.MG?16:r===ROLE.GRENADIER?13:r===ROLE.MARKSMAN?5:7}
function fireCadence(r){return r===ROLE.MG?2:r===ROLE.MARKSMAN?5:r===ROLE.SCOUT?4:r===ROLE.GRENADIER?6:3}
...
function init(n=count){
  ...
  activity={shots:0,heals:0,grenades:0,scoutShots:0,mgBursts:0,engaged:0,kills:0};combatFx=[];deathFx=[];
  ...
  pushLog('SYSTEM',`เริ่ม AI vs AI ${count.toLocaleString()} entities • BLUE บุก / RED รับ`);
  pushLog('COMBAT','ระยะยิงถูกขยายและหน่วยจะหยุดเข้าปะทะเมื่อพบศัตรู');
  draw();updateHUD();
}
...
function buildHierarchy(team,start,end){
  ...
  for(let b=0;b<bnCount;b++){
    ...
    const bn={id,team,start:bs,end:be,homeX:side*.72,homeY:lane,targetX:team===0?-.52:.16,targetY:lane,order:team===0?'ASSEMBLE':'DEFEND',state:'READY'};hierarchy.battalions.push(bn);
    ...
    for(let c=0;c<coCount;c++){
      ...
      for(let q=0;q<sqCount;q++){
        ...
        const sq={id:sqId,team,battalionId:id,companyId:coId,start:qs,end:qe,anchorX:side*.72+side*(c*.032+qc*.013),anchorY:lane+coY+(qr-.5)*.025,targetX:side*.58,targetY:lane};hierarchy.squads.push(sq);
...
function pushLog(type,text){log.push({tick:world?.tick??0,type,text});if(log.length>80)log.shift();$('log').innerHTML=log.slice(-16).reverse().map(x=>`<div class="entry"><b>${type==='CONTACT'?'⚔️ ':''}${x.type}</b> • ${x.text}<div class="muted">tick ${x.tick}</div></div>`).join('')}
...
function nearestEnemy(i,sp,maxRange=.24){const rings=Math.ceil(maxRange/sp.cell),r2=maxRange*maxRange;let best=-1,bestD=r2;for(const j of neighbors(i,sp,rings)){if(j===i||!alive(j)||world.team[j]===world.team[i])continue;const dx=world.x[j]-world.x[i],dy=world.y[j]-world.y[i],d2=dx*dx+dy*dy;if(d2<bestD){bestD=d2;best=j}}return{entity:best,d2:bestD}}
...
function autonomousCommand(){
  ...
  for(const bn of hierarchy.battalions){const st=battalionStats(bn),local=(bn.id%(Math.max(1,hierarchy.battalions.filter(x=>x.team===bn.team).length))),laneBias=((local%3)-1)*.10;
    if(bn.team===0){
      ...
      else{bn.order=doctrine==='AGGRESSIVE'?'ASSAULT':doctrine==='CAUTIOUS'?'BOUND_ADVANCE':'ATTACK';bn.targetX=doctrine==='CAUTIOUS'?-.03:.06;bn.targetY=Math.max(-.68,Math.min(.68,bn.homeY*.35+laneBias));bn.state='ADVANCING';}
    }else{
      const pressure=teamNearHill(0)>18;
      if(st.strength<.27||st.morale<17){bn.order='FALLBACK';bn.targetX=.62;bn.targetY=bn.homeY;bn.state='ROUTING';}
      else if(pressure&&st.morale>48&&local%3===0){bn.order='COUNTER';bn.targetX=-.01;bn.targetY=bn.homeY*.25;bn.state='COUNTERATTACK';}
      else{bn.order='DEFEND';bn.targetX=.08+(local%2)*.04;bn.targetY=Math.max(-.64,Math.min(.64,bn.homeY*.55));bn.state='HOLDING';}
    }
  }
...
function movementProposal(s,sp){
  const t0=performance.now();if(world.tick%5===0)autonomousCommand();updateSquadTargets();const centers=computeSquadCenters(),writes=[],step=.0105;
  let inContact=0;
  for(let i=0;i<count;i++)if(alive(i)){
    ...
    const slotErr=Math.hypot(world.x[i]-tx,world.y[i]-ty),centerErr=Math.hypot(world.x[i]-centers.sx[sq.id],world.y[i]-centers.sy[sq.id]);cohesion[i]=Math.max(0,100-Math.min(100,slotErr*400+centerErr*150));
    const contact=nearestEnemy(i,sp,.19),contactDist=contact.entity>=0?Math.sqrt(contact.d2):999;
    let contactMul=1;if(contactDist<roleRange(r)*.90){contactMul=bn.order==='ASSAULT'?.28:.08;inContact++;}
    if(suppression[i]>65)contactMul*=.35;
    const dx=tx-world.x[i],dy=ty-world.y[i],d=Math.hypot(dx,dy);if(d<.0025||contactMul===0)continue;
    const orderMul=bn.order==='RETREAT'||bn.order==='FALLBACK'?1.18:bn.order==='ASSAULT'?1.08:1,roleMul=r===ROLE.SCOUT?1.06:r===ROLE.MG?.90:1,m=Math.min(step*contactMul*orderMul*roleMul*(.35+.65*morale[i]/100)*(1-Math.min(.78,suppression[i]/130))*(.68+.32*cohesion[i]/100),d)/d;
...
function roleSystemsProposal(s,sp){
  const t0=performance.now(),writes=[],events=[];activity={shots:0,heals:0,grenades:0,scoutShots:0,mgBursts:0,engaged:activity.engaged||0,kills:0};
  combatFx=[];
  ...
  for(let i=0;i<count;i++)if(alive(i)){
    ...
    if(r===ROLE.LEADER&&world.tick%5===0){const sq=hierarchy.squads[squadId[i]];for(let j=sq.start;j<sq.end;j++)if(alive(j)){morale[j]=Math.min(100,morale[j]+.15);cohesion[j]=Math.min(100,cohesion[j]+.22)}}
    if(r===ROLE.MEDIC&&world.tick%4===0){let best=-1,bhp=101;for(const j of neighbors(i,sp,2)){if(j!==i&&alive(j)&&world.team[j]===world.team[i]&&world.hp[j]<bhp&&world.hp[j]<88){best=j;bhp=world.hp[j]}}if(best>=0){addHP(best,2.8,i,'medic');morale[best]=Math.min(100,morale[best]+.25);activity.heals++}}
    if(((world.tick+i*7)%cad)!==0)continue;
    const range=roleRange(r),target=nearestEnemy(i,sp,range);if(target.entity<0)continue;const best=target.entity,bestD=target.d2;
    const distance=Math.sqrt(bestD),accuracy=Math.max(.40,1-distance/range),coh=.55+.45*cohesion[i]/100,base=roleDamage(r),dmg=Math.max(.8,base*(.60+.40*morale[i]/100)*(1-Math.min(.68,suppression[i]/150))*coh*(.70+.30*accuracy));
    if(r===ROLE.GRENADIER&&world.tick%12===0){let hit=0;for(const j of neighbors(best,sp,1)){if(hit>=6)break;if(alive(j)&&world.team[j]!==world.team[i]){const dx=world.x[j]-world.x[best],dy=world.y[j]-world.y[best];if(dx*dx+dy*dy<.035*.035){addHP(j,-dmg*.85,i,'grenade');suppression[j]=Math.min(100,suppression[j]+14);morale[j]=Math.max(0,morale[j]-1.6);hit++}}}if(hit){activity.grenades++;combatFx.push({x1:world.x[i],y1:world.y[i],x2:world.x[best],y2:world.y[best],kind:'grenade'});continue}}
    const defense=role[best]===ROLE.ENGINEER&&world.team[best]===1?.84:1;addHP(best,-dmg*defense,i,'fire');suppression[best]=Math.min(100,suppression[best]+roleSuppression(r));morale[best]=Math.max(0,morale[best]-(dmg*.20+suppression[best]*.003));activity.shots++;if(r===ROLE.MG)activity.mgBursts++;if(r===ROLE.SCOUT)activity.scoutShots++;
    if(combatFx.length<120)combatFx.push({x1:world.x[i],y1:world.y[i],x2:world.x[best],y2:world.y[best],kind:r===ROLE.MG?'mg':r===ROLE.MARKSMAN?'marksman':'fire'});
  }
  for(let i=0;i<count;i++)if(alive(i)){suppression[i]=Math.max(0,suppression[i]-.62);if(suppression[i]<15)morale[i]=Math.min(100,morale[i]+.025)}
...
function oneTick(){
  ...
  let deaths=0;for(let i=0;i<count;i++)if(world.hp[i]<=0&&world.active[i]){deathFx.push({x:world.x[i],y:world.y[i],ttl:16});world.hp[i]=0;world.active[i]=0;morale[i]=0;suppression[i]=0;cohesion[i]=0;deaths++}activity.kills=deaths;
  if(deaths>0&&world.tick%5===0)pushLog('CONTACT',`${deaths} casualties ใน tick นี้ • กำลังยิง ${activity.shots} นัด/ชุด`);
  updateHill();if(world.tick%20===0){const b=teamStats(0),r=teamStats(1);pushLog('SITREP',`BLUE ${b.alive} • RED ${r.alive} • ยิง ${activity.shots} • engaged ${activity.engaged} • Hill ${hillState()}`)}updateHUD();
}
...
function draw(){if(!world)return;const w=canvas.width,h=canvas.height,sx=x=>(x+1)*.5*w,sy=y=>(y+1)*.5*h;ctx.clearRect(0,0,w,h);ctx.fillStyle='#0b1925';ctx.fillRect(0,0,w,h);ctx.strokeStyle='#1e3447';ctx.lineWidth=1;for(let x=0;x<=10;x++){ctx.beginPath();ctx.moveTo(x*w/10,0);ctx.lineTo(x*w/10,h);ctx.stroke()}for(let y=0;y<=8;y++){ctx.beginPath();ctx.moveTo(0,y*h/8);ctx.lineTo(w,y*h/8);ctx.stroke()}ctx.fillStyle='#6c5934';ctx.beginPath();ctx.arc(sx(0),sy(0),56,0,Math.PI*2);ctx.fill();ctx.fillStyle='#e4c677';ctx.font='bold 18px system-ui';ctx.textAlign='center';ctx.fillText('▲ HILL 42',sx(0),sy(0)+6);
  ...
  ctx.lineWidth=1.5;for(const f of combatFx){ctx.strokeStyle=f.kind==='grenade'?'#ffd166':f.kind==='mg'?'#ffb347':'#ffffffaa';ctx.beginPath();ctx.moveTo(sx(f.x1),sy(f.y1));ctx.lineTo(sx(f.x2),sy(f.y2));ctx.stroke();if(f.kind==='grenade'){ctx.fillStyle='#ff9f4388';ctx.beginPath();ctx.arc(sx(f.x2),sy(f.y2),8,0,Math.PI*2);ctx.fill()}}
