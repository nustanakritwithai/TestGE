...
function buildHierarchy(team,start,end){
  ...
  for(let b=0;b<teamBnCount;b++){
    ...
    const lane=teamBnCount===1?0:-0.72+b*(1.44/(teamBnCount-1));
    ...
    for(let c=0;c<companyCount;c++){
      ...
      const coYOffset=(c-(companyCount-1)/2)*.08;
      ...
      for(let q=0;q<squadCount;q++){
        ...
        const sq={id:sqId,team,battalionId:bnId,companyId:coId,start:qStart,end:qEnd,anchorX:bn.anchorX+side*(c*.035+qCol*.014),anchorY:bn.anchorY+coYOffset+(qRow-.5)*.026,targetX:bn.targetX,targetY:bn.targetY};hierarchy.squads.push(sq);
...
function updateFormationTargets(){
  for(const bn of hierarchy.battalions){const side=bn.team===0?-1:1;bn.targetX=attackOrder?side*.10:side*.58;bn.targetY=bn.anchorY*.55;}
  for(const sq of hierarchy.squads){const bn=hierarchy.battalions[sq.battalionId],co=hierarchy.companies[sq.companyId],side=sq.team===0?-1:1;const localSq=Math.floor((sq.start-co.start)/SQUAD_SIZE),row=Math.floor(localSq/5),col=localSq%5;sq.targetX=bn.targetX+side*(co.id%3)*.018+side*col*.012;sq.targetY=bn.targetY+(row-.5)*.032+(co.id%3-1)*.065;}
}
...
function computeSquadCenters(){const sx=new Float64Array(hierarchy.squads.length),sy=new Float64Array(hierarchy.squads.length),sc=new Uint16Array(hierarchy.squads.length);for(let i=0;i<count;i++)if(alive(i)){const q=squadId[i];sx[q]+=world.x[i];sy[q]+=world.y[i];sc[q]++;}for(let q=0;q<hierarchy.squads.length;q++)if(sc[q]){sx[q]/=sc[q];sy[q]/=sc[q];}return {sx,sy,sc};}

function movementProposal(s){
  const t0=performance.now();updateFormationTargets();const centers=computeSquadCenters(),writes=[],step=.0115;
  for(let i=0;i<count;i++)if(alive(i)){
    ...
    const tx=sq.targetX+side*col*.0075,ty=sq.targetY+(row-.5)*.013;slotX[i]=tx;slotY[i]=ty;
    const centerError=Math.hypot(world.x[i]-centers.sx[sq.id],world.y[i]-centers.sy[sq.id]),slotError=Math.hypot(world.x[i]-tx,world.y[i]-ty);
    ...
    writes.push({entity:i,field:'x',value:nx,expectedVersion:s.version[i],expectedGeneration:s.generation[i]},{entity:i,field:'y',value:ny,expectedVersion:s.version[i],expectedGeneration:s.generation[i]});bn.cohesion=bn.cohesion*.995+cohesion[i]*.005;
  }
  profiler.move=performance.now()-t0;return makeProposal({snapshot:s,systemId:'totalfront.formation-movement',source:'formation-hierarchy',priority:120,writes,events:[],meta:{authority:true,battalions:hierarchy.battalions.length,squads:hierarchy.squads.length}});
}
...
function combatProposal(s,sp){
  ...
  for(let i=0;i<count;i++)if(alive(i)){
    ...
    if(best<0)continue;shots++;const distance=Math.sqrt(bestD),base=2+(1-distance/range)*2.4,coh=.55+.45*cohesion[i]/100,dmg=Math.max(.45,base*(.55+.45*morale[i]/100)*(1-Math.min(.65,suppression[i]/150))*coh);
  ...
  profiler.combat=performance.now()-t0;return makeProposal({snapshot:s,systemId:'totalfront.combat',source:'small-arms',priority:220,writes,events,meta:{authority:true,shots,queries:profiler.queries,candidates:profiler.candidates}});
}

function oneTick(){
  const t0=performance.now(),s0=performance.now(),s=snapshot(world);profiler.snap=performance.now()-s0;const sp=buildSpatial(),move=movementProposal(s),combat=combatProposal(s,sp);
  ...
  if(world.tick%25===0){const b=teamStats(0),r=teamStats(1);pushLog('SITREP',`BLUE ${b.alive} • RED ${r.alive} • cohesion ${b.cohesion.toFixed(0)}/${r.cohesion.toFixed(0)} • ${profiler.cells} cells`)}updateHUD();
}
...
$('pause').onclick=()=>{running=!running;$('pause').textContent=running?'⏸ PAUSE':'▶ RESUME'};$('speed').onclick=()=>{speed=speed===1?2:speed===2?4:1;$('speed').textContent=`▶ ${speed}×`};$('order').onclick=()=>{attackOrder=!attackOrder;$('order').textContent=attackOrder?'🛡 HOLD LINE':'⚔️ ATTACK HILL';pushLog('ORDER',attackOrder?'Battalion ทั้งสองฝ่ายรุกเข้าสู่ Hill 42':'หยุดการรุกและกลับ formation line')};$('reset').onclick=()=>init(count);
