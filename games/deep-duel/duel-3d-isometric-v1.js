...
function buildArena(){
 ...
 // curved wall approximation + stands
 ...
 // gates
 for(const z of [-7.0,7.0]){const g=mesh(new THREE.BoxGeometry(3.1,2.8,.55),MAT.dark,0,1.25,z);const arch=mesh(new THREE.TorusGeometry(1.5,.32,10,28,Math.PI),MAT.stone,0,2.55,z+(z>0?-.32:.32));arch.rotation.x=Math.PI/2;arch.rotation.z=Math.PI;}
 // pillars / braziers
 [[-2.7,.2],[2.7,-.2],[-4.5,3.2],[4.5,-3.2]].forEach(([x,z],idx)=>{
  ...
  if(idx>1){const bowl=mesh(new THREE.CylinderGeometry(.42,.25,.18,16),MAT.bronze,x,2.67,z);const flame=mesh(new THREE.SphereGeometry(.18,10,8),new THREE.MeshStandardMaterial({color:0xff8a22,emissive:0xff4a00,emissiveIntensity:4}),x,2.92,z);const pl=new THREE.PointLight(0xff6a22,7,6);pl.position.set(x,3,z);scene.add(pl)}
 });
 // raised dais north/south
...
function fighterModel(side){
 ...
 const ring=new THREE.Mesh(new THREE.RingGeometry(.42,.49,32),new THREE.MeshBasicMaterial({color:side==='a'?0x57bfff:0xff625e,transparent:true,opacity:.9,side:THREE.DoubleSide}));ring.rotation.x=-Math.PI/2;ring.position.y=.025;g.add(ring);
 g.userData={swordArm,baseSwordRot:swordArm.rotation.z,ring};shadow(g);scene.add(g);return g;
}
...
function move(f,enemy,dt){if(f.current&&f.phase!=='IDLE'&&f.current.kind==='attack')return;const t=f.targetPoint||{x:enemy.x,z:enemy.z},dx=t.x-f.x,dz=t.z-f.z,len=Math.hypot(dx,dz)||1,edx=enemy.x-f.x,edz=enemy.z-f.z,ed=Math.hypot(edx,edz)||1;let vx=dx/len,vz=dz/len;if(f.intent==='CLOSE_DISTANCE'&&ed>1.9){vx=edx/ed;vz=edz/ed}if(f.intent==='SURVIVE'&&ed<2.5){vx=-edx/ed;vz=-edz/ed}const speed=f.mobility*(f.stamina<20?.55:1),nx=f.x+vx*speed*dt/1000,nz=f.z+vz*speed*dt/1000;let p=clampArena(nx,nz);if(blocked(p.x,p.y)){const s=f.side==='a'?1:-1;p=clampArena(f.x-vz*s*speed*dt/1000,f.z+vx*s*speed*dt/1000)}f.vx=vx*speed;f.vz=vz*speed;f.x=p.x;f.z=p.y;f.y=terrainHeight(f.x,f.z);f.zone=zoneAt(f.x,f.z)}
...
function syncModel(f,e,dt){const m=f.model;m.position.set(f.x,f.y,f.z);m.rotation.y=Math.atan2(e.x-f.x,e.z-f.z);const bob=Math.sin(performance.now()*.008+(f.side==='a'?0:1))*0.025;m.position.y+=bob;if(f.hitFlash>0){f.hitFlash-=dt;m.scale.setScalar(1+.06*Math.sin(f.hitFlash*.08))}else m.scale.setScalar(1);}
...
function frame(t){const dt=Math.min(34,t-lastT||16);lastT=t;if(running&&!finished){aiClock+=dt;if(aiClock>=250){aiClock=0;aiTick()}move(A,B,dt);move(B,A,dt);updateAction(A,B,dt);updateAction(B,A,dt);A.stamina=clamp(A.stamina+dt*.004,0,100);B.stamina=clamp(B.stamina+dt*.004,0,100)}if(A&&B){syncModel(A,B,dt);syncModel(B,A,dt);updateFx(dt);renderHUD()}renderer.render(scene,camera);requestAnimationFrame(frame)}
